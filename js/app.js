...
(function () {
  ...
  var VERSION = (function () { try { return API.runtime.getManifest().version; } catch (e) { return '2.0.0'; } })();
  ...
  function applyTheme() {
    ...
    try {
      localStorage.setItem('shelf.boot', JSON.stringify({ theme: s.theme, glass: s.glass, light: Core.computeTheme(s, false), dark: Core.computeTheme(s, true), layout: layout }));
  ...
  function pageLayout() {
    if (!IS_PAGE) { html.style.setProperty('--hcols', '4'); return false; }
    ...
    var pad = vw < 600 ? 32 : 72;
    var cols = Core.clamp(Math.floor((w - pad) / cell), 3, 12);
    var lc = s.listColumns === 1 ? Core.clamp(Math.floor(w / 380), 1, 3) : Core.clamp(Math.min(s.listColumns, Math.floor(w / 280)), 1, 3);
    var changed = cols !== PAGE.cols || lc !== PAGE.list || ic !== PAGE.icon;
    PAGE = { cols: cols, list: lc, icon: ic, w: w };
  ...
  function measure(img) {
    try {
      ...
      return { opaque: opaque, used: used };
    } catch (e) { return null; }
  }
  ...
  function toResult(c, r) {
    ...
    var res = { src: c.src, bleed: !!c.bleed && square, tiny: false, fit: null };
    if (r.shape) {
  ...
  function buildIcon(app) {
    ...
    if (type === 'letter') mono();
      ...
      var c = app.cache;
      if (c && c.url === app.url && c.src) show(c.src, c);
  ...
  function fill(container, list, horizontal) {
    var s = state.settings, perPage = s.view === 'grid' ? s.rows * visCols() : s.rows;
    ...
    list.forEach(function (x, i) {
      var el = isFolder(x) ? buildFolderItem(x) : buildAppItem(x, searching);
      if (horizontal && i % perPage === 0) el.classList.add('snap');
      frag.appendChild(el);
    });
  ...
  function renderEmpty() {
    ...
    emptyEl.textContent = '';
    emptyEl.append(
      h('div', { class: 'empty-art', 'aria-hidden': 'true' }, h('span'), h('span'), h('span'), h('span')),
  ...
  function renderNoResults(q) {
    ...
    var looksUrl = n.ok && /[.:]/.test(q);
    noResEl.textContent = '';
    noResEl.append(h('h2', null, 'No Results'), h('p', null, 'No apps match “' + q + '”.'),
  ...
  function launchFx(id, background) {
    if (REDUCED) return;
    var el = doc.querySelector('.layer.folder-layer .app[data-id="' + id + '"] .icon') || appsEl.querySelector('.app[data-id="' + id + '"] .icon');
  ...
  function openApp(app, opts) {
    ...
    app.last = Date.now();
    if (!opts.quiet) launchFx(app.id, background);
    ...
    if (!API || !API.tabs) {
      ...
      if (tracked) save({ noSync: true, ifRev: state.rev });
  ...
  function showMenu(x, y, items, opts) {
    ...
    close = openLayer(menu, 'menu', { onClose: function () { rootEl.style.minHeight = ''; window.removeEventListener('resize', place); } });
  ...
  function itemMenu(entry, item, x, y) {
    ...
    return showMenu(x, y, [
      ...
      '-',
      { label: 'Remove', icon: 'trash', danger: true, run: function () { removeApp(entry, doc.querySelector('.app[data-id="' + entry.id + '"]')); } },
    ]);
  }
    ...
    folderBtn.addEventListener('click', function () {
      ...
      showMenu(r.left, r.bottom + 6, [
        ...
        { label: 'New Folder…', icon: 'folderPlus', run: function () {
          ...
          promptDialog({ title: 'New Folder', placeholder: 'Folder name', value: n0.ok ? Core.categoryOf(n0.url) || '' : '', confirm: 'Create' }).then(function (name) {
            if (name == null) return;
  ...
  function seg(key, options, label) {
    ...
    var btns = options.map(function (o) {
      var b = h('button', { type: 'button', role: 'radio', onclick: function () { var p = {}; p[key] = o[0]; setSetting(p); } }, o[1]);
      el.appendChild(b);
      return b;
    });
  ...
  function toggle(key, label) {
    ...
    input.addEventListener('change', function () { var p = {}; p[key] = input.checked; setSetting(p); });
  ...
  function row(ic, color, label, control, opts) {
    ...
    var r = h('div', { class: 'srow' + (ic ? ' has-icon' : '') }, ic ? sicon(ic, color) : null, lbl, control);
  ...
  function wallpaperPicker() {
    ...
    var btns = Object.keys(Core.WALLPAPERS).map(function (key) {
      var prev = h('span', { class: 'prev wp' }, h('i'), h('i'));
      var b = h('button', { type: 'button', class: 'pal', role: 'radio', onclick: function () { setSetting({ wallpaper: key }); } }, prev, Core.WALLPAPERS[key].name);
      wrap.appendChild(b);
      return [key, b, prev];
    });
    updaters.push(function () {
  ...
  function webInstallRow() {
    ...
    var r = h('button', { type: 'button', class: 'srow btn-row has-icon', onclick: function () {
      ...
      if (installPrompt) { installPrompt.prompt(); installPrompt.userChoice.then(function () { installPrompt = null; updateSettings(); }).catch(function () {}); return; }
      dialog({ title: 'Install Shelf', message: IS_IOS
        ? 'In Safari, tap the Share button, then “Add to Home Screen”. Shelf will open full screen, like an app, and your data is better protected.'
        : 'Open your browser menu and choose “Install app” or “Add to Home screen”.', buttons: [{ label: 'OK', value: true, style: 'bold' }] });
  ...
  function handleIncoming() {
    ...
    if (dup) { toast('“' + dup.app.name + '” is already on your shelf'); return; }
    openAppSheet(null, { url: nrm.url, name: name });
  }
    ...
    var body = h('div', { class: 'settings-body' },
      ...
      h('div', { class: 'group' },
        ...
        row('sliders', '#8E8E93', 'Custom Colors', (function () {
          ...
          input.addEventListener('change', function () { var c = JSON.parse(JSON.stringify(state.settings.custom)); c.enabled = input.checked; setSetting({ custom: c }); });
      ...
      h('div', { class: 'group-title' }, 'Layout'),
      h('div', { class: 'group' },
        row('grid', '#0A84FF', 'View', seg('view', [['grid', 'Icons'], ['list', 'List']], 'View')),
        row('arrows', '#30D158', 'Direction', seg('orientation', [['vertical', 'Vertical'], ['horizontal', 'Horizontal']], 'Direction')),
        row('cols', '#40C8E0', 'Apps per Row', stepper(), { dynLabel: function () {
          return state.settings.orientation === 'horizontal' ? 'Apps per Column' : (state.settings.view === 'list' ? 'Columns' : 'Apps per Row');
        } }),
        stackRow('sort', '#5856D6', 'Sort', (function () { var el = seg('sort', [['manual', 'Manual'], ['name', 'A–Z'], ['usage', 'Most Used']], 'Sort'); el.classList.add('wide'); return el; })()),
        row('size', '#FF375F', 'Icon Size', seg('iconSize', [['small', 'S'], ['medium', 'M'], ['large', 'L']], 'Icon size')),
        row('shape', '#BF5AF2', 'Icon Shape', seg('iconShape', [['rounded', 'Rounded'], ['circle', 'Circle']], 'Icon shape')),
        row('text', '#FF9F0A', 'Show Names', toggle('showNames', 'Show names'), { disabledWhen: function () { return state.settings.view === 'list'; } })),
      (function () {
        var f = h('div', { class: 'group-foot' });
        updaters.push(function () {
          var s = state.settings;
          f.textContent = (s.view === 'list' ? 'Names are always shown in List view. Use “Move to Folder” to organize.' : s.sort !== 'manual' ? 'Rearranging switches back to Manual order.' : 'Tip: drag one app onto another to make a folder.') +
      ...
      h('div', { class: 'group' },
        ...
        row('tab', '#30D158', 'Open in Background', toggle('openInBackground', 'Open in background'), { hiddenWhen: function () { return IS_WEB; } })),
      h('div', { class: 'group-foot' }, IS_WEB
        ? 'Apps open in a new tab. Tip: install Shelf to your home screen for a full-screen app, and share any page to Shelf to add it.'
        : 'Apps always open in a new tab. ' + mod + '-click or middle-click opens in the background; Shift-click opens a new window. Right-click any web page → “Add Page to Shelf”. Type “sh” + space in the address bar to search your apps.'),
      ...
      h('div', { class: 'safe-note' }, icon('shield'), h('span', null,
        IS_WEB ? 'Shelf keeps your last 10 versions automatically and asks the browser for protected storage. For the strongest protection, install it to your home screen and export a backup.'
      ...
      h('div', { class: 'group' },
        ...
        btnRow('keyboard', '#636366', 'Keyboard Shortcuts', showShortcuts),
        IS_PAGE ? null : btnRow('expand', '#0A84FF', 'Open in Full Page', openFullPage, { external: true }),
        CONFIG.feedbackUrl ? btnRow('chat', '#30D158', 'Send Feedback', function () { openApp({ name: 'Feedback', url: CONFIG.feedbackUrl }); }, { external: true }) : null),
  ...
  function showShortcuts() {
    ...
    var rows = [
      ['Search', 'Just type, or /'], ['Open first result', 'Enter'], ['Open app 1–9', alt + ' + 1–9'],
  ...
  function exportData() {
    ...
    try {
      var blob = new Blob([JSON.stringify(Core.exportPayload(state.items, state.settings, VERSION), null, 2)], { type: 'application/json' });
  ...
  function importData() {
    ...
    input.addEventListener('change', function () {
      ...
      reader.onload = function () {
        ...
        var msg = 'Found ' + plural(n, 'app') + (nf ? ' in ' + plural(nf, 'folder') : '') + (parsed.skipped ? ' (' + parsed.skipped + ' invalid skipped)' : '') + '.' +
        ...
        dialog({ title: 'Import Backup?', message: msg, buttons: buttons }).then(function (choice) {
          ...
          snapshot('Before import').then(function () {
            if (choice === 'merge') {
              ...
              replaceAll(state.items, null, added ? 'Added ' + plural(added, 'app') : 'Everything was already on your shelf');
            } else replaceAll(parsed.items, parsed.settings, 'Imported ' + plural(n, 'app'));
          });
        });
      };
      reader.readAsText(f);
  ...
  function resetAll() {
    dialog({ title: 'Reset Everything?', message: 'This removes all your apps and folders and restores default settings. A copy is kept in Restore Previous Version.',
      ...
      snapshot('Before reset').then(function () {
        iconMem.clear();
        replaceAll(seedItems(), Core.sanitizeSettings(null, DEFAULTS), 'Shelf has been reset');
      });
    });
  }
  function openRestoreSheet() {
    Promise.all([Store.getSnapshots(), Store.readSync()]).then(function (r) {
      ...
      var shell;
      function restoreRow(title, sub, getItems, settings) {
        ...
        b.addEventListener('click', function () {
          dialog({ title: 'Restore This Version?', message: 'Your current apps will be replaced. A copy of them is saved first, so you can switch back.',
            ...
            if (!ok) return;
            snapshot('Before restore').then(function () { shell.close(true); replaceAll(getItems(), settings, 'Restored'); });
          });
        });
        list.appendChild(b);
      }
      ...
      snaps.forEach(function (s) {
        restoreRow(s.reason || 'Automatic', ago(s.at) + ' · ' + plural(s.apps || 0, 'app') + (s.folders ? ', ' + plural(s.folders, 'folder') : ''),
          function () { return Core.sanitizeItems(s.items); }, s.settings);
      });
      shell = sheetShell({
  ...
  function stepInfo() {
    var s = state.settings;
    if (s.orientation === 'horizontal') return { horizontal: true, rows: s.rows };
  ...
  function bindGrid(container, ctx) {
    container.addEventListener('click', function (e) {
      ...
      openApp(l.entry, { background: e.metaKey || e.ctrlKey || state.settings.openInBackground, newWindow: e.shiftKey });
    });
  ...
  function reorderTo(d, target) {
    ...
    var from = d.index;
    flip(d.container, function () { if (target > from) el.after(d.item); else el.before(d.item); });
    d.index = target;
  }
  function autoScroll() {
  ...
  function moveByKey(item, key, ctx) {
    ...
    if (!st.horizontal) delta = key === 'ArrowLeft' ? -1 : key === 'ArrowRight' ? 1 : key === 'ArrowUp' ? -st.cols : st.cols;
    else delta = key === 'ArrowUp' ? -1 : key === 'ArrowDown' ? 1 : key === 'ArrowLeft' ? -st.rows : st.rows;
  ...
  homeEl.addEventListener('click', function (e) { if (state.editing && !e.target.closest('.app') && !e.target.closest('.topbar')) exitEdit(); });
  ...
  doc.addEventListener('keydown', function (e) {
    ...
    if (inSearch) {
      if (e.key === 'Enter') {
        ...
        var bg = e.metaKey || e.ctrlKey || state.settings.openInBackground;
        ...
        else if (q && Core.normalizeUrl(q).ok) openAppSheet(null, { url: q });
  ...
  function checkRemote() {
    ...
    Store.readSync().then(function (sb) {
      if (!sb) { if (state.items.length) requestSync(); return; }
      if (JSON.stringify(sb.compact) === JSON.stringify(Core.compactForSync(state.items))) return;
      if (sb.dev !== state.device && sb.at > (state.updatedAt || 0)) applyRemote(sb);
      else requestSync();
    });
  }
  if (API && API.storage && API.storage.onChanged) {
    API.storage.onChanged.addListener(function (changes, area) {
      ...
      if (area === 'sync' && changes['sb.meta']) {
        var m = changes['sb.meta'].newValue;
        if (m && m.dev !== state.device) checkRemote();
      }
    });
  } else {
    window.addEventListener('storage', function (e) { if (e.key === Store.KEYS.meta) reloadFromStorage(); });
  }
  ...
  function detectCurrentTab() {
    ...
    return call(API.tabs, 'query', { active: true, currentWindow: true }).then(function (tabs) {
      var t = tabs && tabs[0];
      if (!t || !/^https?:/i.test(t.url || '')) return;
      var n = Core.normalizeUrl(String(t.url).split('#')[0]);
      if (!n.ok) return;
      if (Core.flatten(state.items).some(function (e) { return Core.sameUrl(e.app.url, n.url) || Core.hostOf(e.app.url) === Core.hostOf(n.url); })) return;
      state.currentTab = { url: n.url, title: String(t.title || ''), favIconUrl: Core.isSafeImageSrc(t.favIconUrl) ? t.favIconUrl : null };
  ...
  function init() {
    ...
    Store.load(DEFAULTS, seedItems, VIEW_ID).then(function (st) {
      ...
      state.updatedAt = st.meta ? st.meta.updatedAt : 0;
      state.rev = st.meta ? st.meta.rev : null;
      if (st.restored === 'sync' && state.items.length) setTimeout(function () { toast('Restored ' + plural(totalApps(), 'app') + ' from browser sync'); }, 300);
      else if (st.restored === 'snapshot') setTimeout(function () { toast('Recovered your apps from a backup'); }, 300);
      ...
      updateSettings();
      state.ready = true;
      if (IS_WEB) { handleIncoming(); idle(function () { requestPersist(false); }); }
      if (state.settings.showSearch && state.settings.autofocusSearch && state.items.length) setTimeout(function () { searchInput.focus({ preventScroll: true }); }, 30);
      idle(function () { Store.autoSnapshot(state.items, state.settings).catch(function () {}); checkRemote(); });
      return detectCurrentTab();
    }).then(function () { if (state.currentTab && !state.items.length) renderEmpty(); });
  }
  ...
  if (Store.channel) Store.channel.onmessage = function (e) { if (e.data && e.data.origin !== VIEW_ID) reloadFromStorage(); };
