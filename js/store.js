...
(function (root, factory) {
  ...
  var Todos = root.ShelfTodos || (typeof require === 'function' ? require('./todos.js') : null);
  var api = factory(Core, root, Todos);
  if (typeof module === 'object' && module.exports) module.exports = api;
  else root.ShelfStore = api;
})(typeof globalThis !== 'undefined' ? globalThis : this, function (Core, g, Todos) {
  'use strict';
  ...
  var KEYS = {
    ...
    todos: 'shelf.todos', todoNotified: 'shelf.todoNotified',
  };
  var SYNC_META = 'sb.meta', SYNC_PREFIX = 'sb.', CHUNK = 2000, SYNC_MAX = 95000;
  var TD_META = 'td.meta', TD_PREFIX = 'td.', SYNC_QUOTA = 100500;   // chrome.storage.sync allows 102,400 bytes in total
  ...
  function writeSync(items, settings) {
    ...
    return deviceId().then(function (dev) {
      ...
      return syncBytes(TD_PREFIX).then(function (tdBytes) {
      if (payload.length > Math.min(SYNC_MAX, SYNC_QUOTA - tdBytes - 1500)) return recordSync({ ok: false, reason: 'too-large', at: at });
      var obj = {}, n = 0;
  ...
  /* Bytes used in sync by keys with this prefix (key + JSON value, the way the browser counts them). */
  function syncBytes(prefix) {
    if (!sync) return Promise.resolve(0);
    return sync.get(null).then(function (all) {
      var n = 0;
      Object.keys(all || {}).forEach(function (k) { if (k.indexOf(prefix) === 0) { try { n += k.length + JSON.stringify(all[k]).length; } catch (e) { /* ignore */ } } });
      return n;
    }, function () { return 0; });
  }
  ...
  /* ───────── to-dos: local + browser sync (merged per task, newest wins) ───────── */
  function getTodos() {
    return local.get([KEYS.todos]).then(function (d) { return Todos.sanitize(d[KEYS.todos]); }, function () { return Todos.empty(); });
  }
  var todoChain = Promise.resolve();
  function saveTodos(state, opts) {
    opts = opts || {};
    var clean = Todos.sanitize(state);
    var o = {}; o[KEYS.todos] = clean;
    todoChain = todoChain.then(function () { return local.set(o); }, function () { return local.set(o); });
    return todoChain.then(function () {
      if (channel) { try { channel.postMessage({ type: 'todos', origin: opts.origin || '' }); } catch (e) { /* ignore */ } }
      if (!opts.noSync && sync) {
        local.get([KEYS.settings]).then(function (d) {
          var st = d[KEYS.settings];
          if (st && st.sync === false) return;
          if (IS_BG) scheduleSync(opts.syncNow ? 0 : 2000);
          else call(API.runtime, 'sendMessage', { type: 'shelf:sync' }).catch(function () { scheduleSync(1500); });
        });
      }
      return clean;
    });
  }
  function readTodoSync() {
    if (!sync) return Promise.resolve(null);
    return sync.get(null).then(function (all) {
      var m = all[TD_META];
      if (!m || typeof m.n !== 'number' || m.n < 1 || m.n > 40) return null;
      var s = '';
      for (var i = 0; i < m.n; i++) { var c = all[TD_PREFIX + i]; if (typeof c !== 'string') return null; s += c; }
      if (Core.hashStr(s) !== m.h) return null;
      try { return { state: Todos.expand(JSON.parse(s)), at: m.at || 0, dev: m.dev || '' }; } catch (e) { return null; }
    }).catch(function () { return null; });
  }
  var lastTodoBody = null;
  function writeTodoSync(state) {
    if (!sync) return Promise.resolve({ ok: false, reason: 'unsupported' });
    return Promise.all([deviceId(), syncBytes(SYNC_PREFIX)]).then(function (r) {
      var dev = r[0], budget = Math.max(2000, Math.min(SYNC_QUOTA - r[1] - 800, Math.max(30000, SYNC_QUOTA - r[1] - 25000)));   // leave room for more apps
      var body = JSON.stringify(Todos.compact(state, budget - 300));
      if (body === lastTodoBody) return { ok: true, skipped: true };
      var obj = {}, n = 0, at = Date.now();
      for (var i = 0; i < body.length; i += CHUNK) obj[TD_PREFIX + (n++)] = body.slice(i, i + CHUNK);
      if (!n) { obj[TD_PREFIX + '0'] = body; n = 1; }
      obj[TD_META] = { n: n, at: at, dev: dev, h: Core.hashStr(body), v: 1 };
      return sync.set(obj).then(function () {
        lastTodoBody = body;
        return sync.get(null).then(function (all) {
          var stale = Object.keys(all).filter(function (k) { return /^td\.\d+$/.test(k) && Number(k.slice(3)) >= n; });
          return stale.length ? sync.remove(stale) : null;
        }).catch(function () { /* ignore */ });
      }).then(function () { return { ok: true, at: at }; }, function (e) { return { ok: false, reason: 'error', message: String(e && e.message || e) }; });
    });
  }
  var todoReconciling = null;
  function reconcileTodos() {
    if (!sync) return Promise.resolve({ action: 'unsupported' });
    if (todoReconciling) return todoReconciling;
    todoReconciling = Promise.all([readTodoSync(), getTodos(), local.get([KEYS.settings])]).then(function (r) {
      var remote = r[0], mine = r[1], st = r[2][KEYS.settings];
      if (st && st.sync === false) return { action: 'off' };
      if (!remote) return Todos.liveLists(mine).length || mine.tasks.length ? writeTodoSync(mine).then(function () { return { action: 'pushed' }; }) : { action: 'none' };
      var merged = Todos.merge(mine, remote.state);
      var jobs = [];
      var changedLocal = !Todos.equal(merged, mine);
      if (changedLocal) jobs.push(saveTodos(merged, { noSync: true, origin: 'sync' }));
      // Push when the account copy is missing anything we have (compact() may leave out old completed tasks when space is short).
      if (!Todos.equal(Todos.merge(remote.state, {}), merged)) jobs.push(writeTodoSync(merged));
      return Promise.all(jobs).then(function () { return { action: changedLocal ? 'applied' : 'pushed' }; });
    }).catch(function (e) { console.warn('[Shelf] to-do sync failed', e); return { action: 'error' }; })
      .then(function (res) { todoReconciling = null; return res; });
    return todoReconciling;
  }
  function getNotified() { return local.get([KEYS.todoNotified]).then(function (d) { var m = d[KEYS.todoNotified]; return m && typeof m === 'object' && !Array.isArray(m) ? m : {}; }, function () { return {}; }); }
  function setNotified(map) { var o = {}; o[KEYS.todoNotified] = map; return local.set(o).catch(function () {}); }
  ...
  function snapshot(reason, items, settings) {
    items = items || [];
    return Promise.all([getSnapshots(), getTodos()]).then(function (r) {
      var list = r[0], todos = r[1], nTodo = Todos.liveTasks(todos).length;
      if (!Core.countApps(items) && !Core.countFolders(items) && !nTodo) return;
      var entry = { at: Date.now(), reason: reason || 'Automatic', apps: Core.countApps(items), folders: Core.countFolders(items), todos: nTodo, items: slim(items), settings: settings, todoState: todos };
      var first = list[0];
      if (first && JSON.stringify(first.items) === JSON.stringify(entry.items) && JSON.stringify(first.todoState || null) === JSON.stringify(entry.todoState)) return; // unchanged
      list.unshift(entry);
  ...
  function scheduleSync(delay) {
    ...
    syncTimer = setTimeout(function () {
      reconcile().catch(function (e) { console.warn('[Shelf] sync failed', e); })
        .then(function () { return reconcileTodos(); }).catch(function (e) { console.warn('[Shelf] to-do sync failed', e); });
    }, delay || 0);
  }
  ...
  return {
    ...
    getTodos: getTodos, saveTodos: saveTodos, reconcileTodos: reconcileTodos, readTodoSync: readTodoSync, writeTodoSync: writeTodoSync,
    getNotified: getNotified, setNotified: setNotified, syncBytes: syncBytes,
