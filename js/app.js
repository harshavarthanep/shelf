...
(function () {
  ...
  var VERSION = (function () { try { return API.runtime.getManifest().version; } catch (e) { return '2.2.0'; } })();
  ...
  function openInstallGuide() {
    ...
    var shell = sheetShell({ title: 'Install Shelf', left: 'Close', body: [
      h('div', { class: 'install-hero' }, h('div', { class: 'brand-mark big', 'aria-hidden': 'true' }, h('i'), h('i'), h('i'), h('i')),
  ...
  function maybeInstallBanner() {
    ...
    var banner = h('div', { class: 'install-banner', role: 'dialog', 'aria-label': 'Install Shelf' },
      h('div', { class: 'brand-mark', 'aria-hidden': 'true' }, h('i'), h('i'), h('i'), h('i')),
  ...
  function showWelcome(force) {
    ...
    var shell = sheetShell({ title: '', left: 'Skip', onClose: done, body: [
      h('div', { class: 'welcome-hero' },
        h('div', { class: 'brand-mark big', 'aria-hidden': 'true' }, h('i'), h('i'), h('i'), h('i')),
  ...
  function openGuide() {
    ...
    var sections = [
      ...
      ['Find', [
        ['search', '#8E8E93', 'Search', 'Just start typing — even inside folders. Press Enter to open the top result.'],
  ...
  function exportData() {
    ...
    try {
      var blob = new Blob([JSON.stringify(Core.exportPayload(state.items, state.settings, VERSION, logoData), null, 2)], { type: 'application/json' });
  ...
  function hideSplash() {
    ...
    var minMs = REDUCED ? 0 : IS_PAGE ? 620 : 380;
    var wait = Math.max(0, minMs - (window.performance ? performance.now() : 0));
    setTimeout(function () {
      if (!splashEl) return;
      html.classList.add('splash-out');
      setTimeout(function () { html.classList.remove('splash-on', 'splash-out'); if (splashEl) splashEl.remove(); splashEl = null; introAnimation(); }, REDUCED ? 0 : 420);
    }, wait);
  }
  if (splashEl) splashEl.addEventListener('pointerdown', function () { if (splashEl) { html.classList.remove('splash-on', 'splash-out'); splashEl.remove(); splashEl = null; } });
  ...
  function init() {
    ...
    Promise.all([
      ...
      if (IS_WEB && !state.settings.onboarded && !state.items.length && !layers.length) setTimeout(function () { if (!layers.length) showWelcome(false); }, splashing ? 700 : 250);
