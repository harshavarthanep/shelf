...
(function (root, factory) {
  ...
  function sanitizeApp(a) {
    ...
    if (a.hint && isSafeImageSrc(a.hint)) app.hint = a.hint;
  ...
  var ENUMS = {
    ...
    wallpaper: ['aurora', 'sunset', 'ocean', 'meadow', 'dusk', 'mono', 'accent'],
  };
  ...
  function computeTheme(s, dark) {
    ...
    return {
      ...
      '--glass': rgba(b.surface, dark ? 0.1 : 0.4),
      '--glass-strong': rgba(b.surface, dark ? 0.14 : 0.62),
  ...
  function computeLayout(s) {
    ...
    var width;
    if (s.view === 'grid') {
      var cell = icon + (s.showNames ? 26 : 16);
    ...
    width = clamp(Math.round(width), 340, 780);
    return {
      width: width,
      vars: {
        '--popup-w': width + 'px',
        ...
        '--rows': String(s.rows),
        '--tile-r': s.iconShape === 'circle' ? '50%' : '23%',
      },
    };
  }
  ...
  function exportPayload(items, settings, version) {
    function app(a) {
  ...
  function parseImport(text) {
    ...
    if (!count && !countFolders(items)) throw new Error('No valid apps found in this file');
    return { items: items, apps: flatten(items).map(function (e) { return e.app; }), settings: isObj(data) && isObj(data.settings) ? data.settings : null, skipped: Math.max(0, rawCount - count) };
  }
