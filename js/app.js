...
(function () {
  ...
  var VERSION = (function () { try { return API.runtime.getManifest().version; } catch (e) { return '2.3.0'; } })();
  ...
  function dockMax() { return IS_PAGE ? (PAGE.w >= 900 ? 10 : PAGE.w >= 600 ? 8 : 5) : state.settings.windowSize === 'compact' ? 5 : 6; }
  function dockApps() { return Core.flatten(state.items).map(function (e) { return e.app; }).filter(function (a) { return a.dock; }); }
  function setDock(app, on) {
    var a = live(app);
    if (!a) return;
    if (on && dockApps().length >= dockMax()) { toast('The Dock is full — remove an app from it first'); return; }
    if (on) a.dock = true; else delete a.dock;
    haptic();
    save(); renderHome();
    toast(on ? a.name + ' added to the Dock' : a.name + ' removed from the Dock');
  }
  function renderDock() {
    var list = dockApps().slice(0, dockMax());
    var show = state.settings.showDock && list.length > 0 && !state.query.trim();
    dockEl.hidden = !show;
    homeEl.classList.toggle('with-dock', show);
    if (!show) { dockEl.textContent = ''; dockEl.dataset.sig = ''; return; }
    ...
    if (dockEl.dataset.sig === sig) return;
    dockEl.dataset.sig = sig;
    dockEl.textContent = '';
    var shelfEl = h('div', { class: 'dock-shelf' });
    list.forEach(function (a) {
      shelfEl.appendChild(h('a', { class: 'dock-app', href: a.url, draggable: 'false', 'data-id': a.id, title: a.name, 'aria-label': a.name }, buildIcon(a), h('span', { class: 'dock-tip' }, a.name)));
    });
    dockEl.appendChild(shelfEl);
  }
  ...
  dockEl.addEventListener('auxclick', function (e) {
    var it = e.target.closest('.dock-app');
  ...
  dockEl.addEventListener('dragstart', function (e) { e.preventDefault(); });
