...
(function () {
  ...
  var VERSION = (function () { try { return API.runtime.getManifest().version; } catch (e) { return '2.6.0'; } })();
  ...
  function pickBest(cands) {
    return new Promise(function (resolve) {
      ...
      cands.forEach(function (c, i) {
        // Load plainly first (a missing icon never triggers a CORS error); measure with CORS only once it exists.
        var attempt = probe(c.src, 3000, false).then(function (r) {
          if (!r || !c.cors) return r;
          return probe(c.src, 2500, true).then(function (m) { return m || r; });
        });
  ...
  function resolveAuto(app) {
    ...
    var cands = Core.iconCandidates(app.url, app.hint);
    var p = limit(function () {
      return pickBest(cands.filter(function (c) { return c.stage !== 2; })).then(function (best) {
        return best || pickBest(cands.filter(function (c) { return c.stage === 2; }));
      });
    }).then(function (best) {
  ...
  function showDockTip(it) {
    ...
    dockTip.style.top = Math.max(4 + dockTip.offsetHeight, r.top - 14) + 'px';
    // keep the label on screen at the edges
    var half = Math.max(30, dockTip.offsetWidth / 2);
    dockTip.style.left = Math.min(window.innerWidth - half - 4, Math.max(half + 4, r.left + r.width / 2)) + 'px';
  ...
  function renderHome() {
    ...
    emptyEl.hidden = !none || !!(q && todoHits(q).length);
    var tdHits = renderTodoResults(q, list.length);
    noResEl.hidden = none || !q || list.length > 0 || tdHits > 0;
    if (none) renderEmpty();
  ...
  function addMenu() {
    ...
    showMenu(p.x, p.y, [
      ...
      { label: 'Timer…', icon: 'clock', run: function () { timerMenu(p.x, p.y); } },
      state.currentTab ? { label: 'Remind Me About This Page…', icon: 'bell', run: function () { pageReminderMenu(p.x, p.y); } } : null,
    ]);
  }
  ...
  function openGuide() {
    ...
    var sections = [
      ...
      ['To-Do', [
        ...
        IS_WEB ? null : ['link', '#40C8E0', 'From anywhere', 'Select text on any page → right-click → Add to To-Do. In the address bar type “sh”, press Space, then “+ Buy milk tomorrow” (or “todo …”). Type “sh todo” to open your lists; to-dos also appear as suggestions. Alt+Shift+A adds the page you’re on to Shelf.'],
      ]],
      ['Find', [
  ...
  function buildSettings() {
    ...
    var body = h('div', { class: 'settings-body' },
      ...
      h('div', { class: 'group' },
        ...
        IS_PAGE ? row('clock', '#FF9F0A', 'Clock & Greeting', toggle('showClock', 'Clock and greeting')) : null,
        IS_PAGE ? btnRow('text', '#34C759', 'Your Name', function () {
          promptDialog({ title: 'Your Name', message: 'Used in the greeting, e.g. “Good morning, Sam”.', placeholder: 'Name', value: state.settings.greetName, confirm: 'Save' }).then(function (v) { if (v != null) setSetting({ greetName: v }); });
        }, { value: function () { return state.settings.greetName || 'Not set'; }, hiddenWhen: function () { return !state.settings.showClock; } }) : null),
      (function () {
  ...
  function openSettings() {
    ...
    settingsEl.inert = false; settingsEl.removeAttribute('inert');
  ...
  function closeSettings() {
    ...
    // Move focus out before hiding, so assistive tech never loses the focused element.
    if (settingsEl.contains(doc.activeElement)) { try { doc.activeElement.blur(); } catch (e) { /* ignore */ } }
    settingsEl.inert = true; settingsEl.setAttribute('inert', '');
    updateChrome();
  ...
  function showShortcuts() {
    ...
    ].concat(IS_WEB ? [] : [['Open Shelf', IS_MAC ? '⌥⇧S' : 'Alt+Shift+S'], ['Add current page', IS_MAC ? '⌥⇧A' : 'Alt+Shift+A'], ['Address bar', 'sh + Space'], ['Quick to-do', 'sh + Space + “+ text”']]);
  ...
  doc.addEventListener('keydown', function (e) {
    ...
    if (inSearch) {
      if (e.key === 'Enter') {
        ...
        else if (q && todoHits(q).length) { var th = todoHits(q)[0]; openTodos(th.list, { task: th.id }); }
  ...
  /* ───── Full page: clock, date & greeting ───── */
  var heroEl = h('div', { class: 'page-hero', hidden: true, 'aria-live': 'off' },
    h('div', { class: 'hero-time' }), h('div', { class: 'hero-sub' }));
  topbar.parentNode.insertBefore(heroEl, topbar.nextSibling);
  var heroTimer = 0;
  function renderHero() {
    var show = IS_PAGE && state.settings.showClock && !state.query.trim() && !state.editing;
    heroEl.hidden = !show;
    clearTimeout(heroTimer);
    if (!show) return;
    var d = new Date(), hr = d.getHours();
    var time, date;
    try { time = d.toLocaleTimeString(undefined, { hour: 'numeric', minute: '2-digit' }); date = d.toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' }); }
    catch (e) { time = hr + ':' + String(d.getMinutes()).padStart(2, '0'); date = d.toDateString(); }
    var greet = hr < 5 ? 'Good night' : hr < 12 ? 'Good morning' : hr < 17 ? 'Good afternoon' : hr < 22 ? 'Good evening' : 'Good night';
    var name = state.settings.greetName;
    var mt = /^(.*?)\s?([AaPp]\.?[Mm]\.?)$/.exec(time);
    heroEl.firstChild.textContent = '';
    if (mt) heroEl.firstChild.append(mt[1], h('span', { class: 'ampm' }, mt[2].replace(/\./g, '').toUpperCase()));
    else heroEl.firstChild.textContent = time;
    var c = todosReady ? TD.counts(todos) : null, extra = '';
    if (c && (c.overdue || c.today)) extra = ' · ' + (c.overdue ? c.overdue + ' overdue' : '') + (c.overdue && c.today ? ', ' : '') + (c.today ? c.today + ' due today' : '');
    heroEl.lastChild.textContent = greet + (name ? ', ' + name : '') + ' · ' + date + extra;
    heroTimer = setTimeout(renderHero, (60 - d.getSeconds()) * 1000 + 50);
  }
  heroEl.addEventListener('click', function () { if (todosReady && TD.liveLists(todos).length) openTodos('today'); });

  /* ───── Home search also finds to-dos ───── */
  var todoResEl = h('section', { class: 'todo-results', hidden: true, 'aria-label': 'Matching to-dos' });
  appsEl.parentNode.insertBefore(todoResEl, dotsEl);
  function todoHits(q) {
    if (!todosReady || !q) return [];
    var f = String(q).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    return TD.liveTasks(todos).filter(function (t) {
      return !t.done && (t.text + ' ' + (t.notes || '')).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').indexOf(f) >= 0;
    }).sort(function (a, b) { return (TD.effectiveDue(a) || Infinity) - (TD.effectiveDue(b) || Infinity); });
  }
  function renderTodoResults(q, appCount) {
    if (!todoResEl) return 0;
    var hits = todoHits(q);
    todoResEl.hidden = !hits.length;
    todoResEl.textContent = '';
    if (!hits.length) return 0;
    var now = Date.now(), e = ENGINES[state.settings.webSearch];
    todoResEl.append(h('div', { class: 'todo-results-title' }, icon('checklist'), 'To-Dos', h('span', null, String(hits.length))));
    hits.slice(0, 6).forEach(function (t) {
      var l = listById(t.list), d = TD.effectiveDue(t);
      todoResEl.appendChild(h('button', { type: 'button', class: 'todo-hit' + (TD.isOverdue(t, now) ? ' over' : ''), style: l ? { '--c': l.color } : null,
        onclick: function () { openTodos(t.list, { task: t.id }); } },
        h('i'), h('span', { class: 't' }, t.text), d ? h('span', { class: 'd' }, TD.fmtDue(d, now)) : null));
    });
    if (!appCount && e) todoResEl.appendChild(h('button', { type: 'button', class: 'todo-hit web', onclick: function () { webSearch(q); } }, icon('globe'), h('span', { class: 't' }, 'Search ' + e[0] + ' for “' + q + '”')));
    return hits.length;
  }
  ...
  function renderTodoDots() {
    ...
    // Never crowd the search bar: how many dots fit depends on the window.
    var vw = window.innerWidth || 400;
    var now = Date.now(), c = TD.counts(todos, now), MAXD = vw < 340 ? 1 : IS_PAGE ? (vw >= 1024 ? 7 : vw >= 600 ? 5 : 3) : (vw <= 380 ? 3 : 4);
    if (lists.length === MAXD + 1) MAXD += 1;   // "+1" would take the same room as the dot itself
    ...
    lists.slice(0, MAXD).forEach(function (l) {
      ...
      todoDotsEl.appendChild(h('button', { type: 'button', class: 'tdot' + (lc.open ? '' : ' is-empty') + (lc.overdue ? ' is-due' : ''), style: { '--c': l.color },
  ...
  function listMenu(id, x, y) {
    ...
    showMenu(x, y, [
      ...
      { label: 'Copy as Text', icon: 'link', run: function () { copyList(id); } },
      '-',
  ...
  /* Quick timers and "remind me about this page" are ordinary to-dos with a reminder — so they sync,
   * notify with Snooze/Done, and show in Today like everything else. */
  function quickReminder(text, due, notes) {
    var r = TD.addTask(todos, { text: text, notes: notes || '', due: due, remind: true, list: defaultListId() });
    if (!r.task) return;
    commitTodos(r.state);
    haptic('success');
    toast('Reminder set for ' + TD.fmtDue(due) + ' (' + TD.fmtRel(due) + ')', { action: 'Undo', onAction: function () { commitTodos(TD.removeTask(todos, r.task.id).state); } });
    maybeAskNotify();
  }
  function timerMenu(x, y) {
    function start(m) { quickReminder('⏱ ' + m + '-minute timer', Date.now() + m * 60e3); }
    showMenu(x, y, [1, 5, 10, 15, 25, 45, 60].map(function (m) { return { label: m === 60 ? '1 Hour' : m + (m === 1 ? ' Minute' : ' Minutes'), icon: 'blank', run: function () { start(m); } }; }).concat(['-', {
      label: 'Custom…', icon: 'clock', run: function () {
        promptDialog({ title: 'Timer', message: 'Minutes (1–1440)', placeholder: '20', confirm: 'Start' }).then(function (v) {
          if (v == null) return;
          var m = Math.round(Number(String(v).replace(/[^\d.]/g, '')));
          if (!m || m < 1 || m > 1440) { toast('Enter 1 to 1440 minutes'); return; }
          start(m);
        });
      } }]), { title: 'Timer' });
  }
  function pageReminderMenu(x, y) {
    var t = state.currentTab;
    if (!t) return;
    var title = Core.cleanText(t.title || '', 200) || Core.displayUrl(t.url);
    var eve = new Date(); eve.setHours(18, 0, 0, 0);
    showMenu(x, y, [
      { label: 'In 1 Hour', icon: 'blank', run: function () { quickReminder(title, Date.now() + TD.HOUR, t.url); } },
      eve.getTime() > Date.now() + 30 * 60e3 ? { label: 'This Evening', icon: 'blank', run: function () { quickReminder(title, eve.getTime(), t.url); } } : null,
      { label: 'Tomorrow Morning', icon: 'blank', run: function () { quickReminder(title, presetTime('tomorrow'), t.url); } },
      { label: 'Next Week', icon: 'blank', run: function () { quickReminder(title, presetTime('week'), t.url); } },
    ], { title: 'Remind Me' });
  }
  function copyList(id) {
    var l = listById(id);
    if (!l) return;
    var lines = [l.name].concat(TD.liveTasks(todos, id).map(function (t) {
      return (t.done ? '[x] ' : '[ ] ') + t.text + (t.due ? ' — ' + TD.fmtDue(TD.effectiveDue(t)) : '') + (t.notes ? '\n    ' + t.notes.replace(/\n/g, '\n    ') : '');
    }));
    copyText(lines.join('\n'), 'Copied ' + l.name);
  }
  ...
  function openTodos(which, opts) {
    ...
    function taskRow(t, now) {
      ...
      var meta = h('div', { class: 'td-meta' },
        due ? h('span', { class: 'td-chip due' + (over ? ' over' : TD.isToday(due, now) ? ' today' : ''), title: new Date(due).toLocaleString() }, icon(t.remind ? 'bell' : 'clock'),
          (t.snooze ? 'Snoozed · ' : '') + TD.fmtDue(due, now) + (!t.done && Math.abs(due - now) < 3 * TD.HOUR ? ' · ' + TD.fmtRel(due, now) : '')) : null,
    ...
    function completeTask(id, row) {
      ...
      if (!t.done) {
        ...
        if (r.repeated) toast('Next: ' + TD.fmtDue(r.next), { action: 'Undo', onAction: function () { commitTodos(TD.updateTask(todos, id, { due: t.due, snooze: t.snooze }).state); } });
    ...
    moreBtn.addEventListener('click', function () {
      ...
      ].concat(l ? ['-', { label: 'Copy as Text', icon: 'link', run: function () { copyList(sel); } }] : []));
    });
  ...
  function openTaskEditor(id) {
    ...
    function paint() {
      ...
      dueNote.textContent = draft.due ? (draft.due < Date.now() ? 'Overdue · ' : '') + TD.fmtDue(draft.due) + ' · ' + TD.fmtRel(draft.due) + (draft.snooze ? ' · snoozed until ' + TD.fmtDue(draft.snooze) : '') : 'No date';
  ...
  function taskMenu(id, x, y) {
    ...
    showMenu(x, y, [
      ...
      { label: 'Duplicate', icon: 'plus', run: function () { var d = TD.addTask(todos, { text: t.text, notes: t.notes, due: t.due, remind: t.remind, prio: t.prio, repeat: t.repeat, list: t.list }); if (d.task) { commitTodos(d.state); toast('Duplicated'); } } },
  ...
  function completeTaskFromAnywhere(id) {
    ...
    var r = TD.toggleDone(todos, id);
    commitTodos(r.state);
    if (!t.done) { haptic('success'); chime(true); toast(r.repeated ? 'Next: ' + TD.fmtDue(r.next) : 'Completed “' + t.text + '”', r.repeated ? { action: 'Undo', onAction: function () { commitTodos(TD.updateTask(todos, id, { due: t.due, snooze: t.snooze }).state); } } : { action: 'Undo', onAction: function () { var c = taskById(id); if (c && c.done) commitTodos(TD.toggleDone(todos, id).state); } }); }
  }
  function snoozeFor(id, until) {
    var before = taskById(id);
    var r = TD.snooze(todos, id, until);
    if (!r.task || !before) return;
    commitTodos(r.state);
    haptic();
    toast('Snoozed until ' + TD.fmtDue(until), { action: 'Undo', onAction: function () { commitTodos(TD.updateTask(todos, id, { due: before.due, snooze: before.snooze, remind: before.remind }).state); } });
  }
  ...
  function showBanner(t) {
    ...
    var b = h('div', { class: 'td-banner', role: 'alert', style: l ? { '--c': l.color } : null },
      h('div', { class: 'td-banner-top' }, brandMark(), h('span', { class: 'td-banner-app' }, 'SHELF · TO-DO'), h('span', { class: 'when' }, 'now'),
