...
(function (root, factory) {
  ...
  function writeSync(items, settings) {
    ...
    return deviceId().then(function (dev) {
      ...
      }).then(function () { return recordSync({ ok: true, at: at }); }, function (e) {
  ...
  function scheduleSync(delay) {
    ...
    syncTimer = setTimeout(function () {
      local.get([KEYS.items, KEYS.settings]).then(function (d) {
        var st = Core.sanitizeSettings(d[KEYS.settings]);
        if (!st.sync || !Array.isArray(d[KEYS.items])) return;
        return writeSync(Core.sanitizeItems(d[KEYS.items]), st);
      }).catch(function (e) { console.warn('[Shelf] sync write failed', e); });
    }, delay || 0);
  }
