...
(function (root, factory) {
  ...
  function iconCandidates(url, hint) {
    ...
    // stage 1: Google's icon services + the icon the browser tab reported (fast, no requests to the site)
    // stage 2: only if those fail, ask the site itself
    var list = [
      { src: 'https://t2.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&fallback_opts=TYPE,SIZE,URL&url=' + encodeURIComponent(u.origin) + '&size=128', min: 32, cors: true, stage: 1 },
      { src: 'https://www.google.com/s2/favicons?domain=' + encodeURIComponent(u.hostname) + '&sz=128', min: 32, stage: 1 },
    ];
    if (hint && isSafeImageSrc(hint)) list.splice(1, 0, { src: hint, min: 16, stage: 1 });
    list.push({ src: u.origin + '/apple-touch-icon.png', min: 57, bleed: true, stage: 2 });
    list.push({ src: u.origin + '/favicon.ico', min: 16, stage: 2 });
    return list;
  }
  ...
  function sanitizeSettings(input, defaults) {
    ...
    return withDerived({
      ...
      greetName: typeof i.greetName === 'string' ? cleanText(i.greetName, 30) : typeof d.greetName === 'string' ? cleanText(d.greetName, 30) : '',
